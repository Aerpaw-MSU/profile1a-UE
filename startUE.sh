...
if [ $MODE == "TESTBED" ]
then
   screen -S UE -dm bash -c "srsue --rf.tx_gain=$TX_GAIN --rf.rx_gain=$RX_GAIN --rf.dl_earfcn=$EARFCN | ts '[%Y-%m-%d %H:%M:%.S]' > srsUE.log" &
elif [ $MODE == "IQ_EMULATION" ]
then
   screen -S UE -dm bash -c "srsue ${ZMQ_ARGS_IQ}"  &
elif [ $MODE == "IQ_DIRECT" ]
then
   screen -S UE -dm bash -c "srsue ${ZMQ_ARGS}"  &
